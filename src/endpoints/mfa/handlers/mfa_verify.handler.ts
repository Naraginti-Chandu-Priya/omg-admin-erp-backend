...
export const mfaVerifyHandler: EndpointHandler<EndpointAuthType.NONE> = async (
  req,
  res
) => {
  ...
  try {
    ...
    if (!user.mfaCode) {
      await transaction.rollback();
      res.status(400).json({ error: MFA_NOT_SETUP });
      return;
    }
    ...
    if (
      ...
      await transaction.rollback();
      res.status(429).json({ error: MFA_ACCOUNT_LOCKED });
      return;
    }
